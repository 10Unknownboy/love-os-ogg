...
const LoveWrapped: React.FC = () => {
  ...
  // Initialize empty, load dynamically on mount
  ...
  const renderStatCard = (stat: any, index: number) => {
    ...
    return (
      ...
      >
        ...
        <p className="text-gray-600 text-sm italic">{stat.subtitle}</p>
      </div>
    );
  };

  // MemoriesSection as your original code
    ...
    <div className="min-h-screen bg-gradient-to-br from-pink-100 via-lavender-50 to-rose-100 overflow-x-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 pb-16">
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-700">Your relationship by the numbers</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {relationshipStats.length > 0 ? (
            relationshipStats.map((stat, idx) => renderStatCard(stat, idx))
          ) : (
            <p>Loading stats...</p>
          )}
        </div>
      ...
      {/* Your original footer and memories modal */}
    </div>
  );
};
